...
public final class ListView extends AndroidViewComponent implements AdapterView.OnItemClickListener {
    ...
    public void SelectionIndex(int index) {
        if (!this.dictItems.isEmpty()) {
            int selectionIndex2 = ElementsUtil.selectionIndex(index, YailList.makeList((List) this.dictItems));
            this.selectionIndex = selectionIndex2;
            if (selectionIndex2 > 0) {
                this.selection = this.dictItems.get(selectionIndex2 - 1).get(Component.LISTVIEW_KEY_MAIN_TEXT).toString();
        ...
        if (listAdapterWithRecyclerView2 != null) {
            listAdapterWithRecyclerView2.toggleSelection(this.selectionIndex - 1);
        }
    }
            ...
            try {
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    if (jsonItem.has(Component.LISTVIEW_KEY_MAIN_TEXT)) {
                        ...
                        if (jsonItem.has(Component.LISTVIEW_KEY_DESCRIPTION)) {
                            str = jsonItem.getString(Component.LISTVIEW_KEY_DESCRIPTION);
                        } else {
                            str = "";
                        }
                        yailItem.put(Component.LISTVIEW_KEY_DESCRIPTION, str);
                        if (jsonItem.has(Component.LISTVIEW_KEY_IMAGE)) {
                            str2 = jsonItem.getString(Component.LISTVIEW_KEY_IMAGE);
                        } else {
                            str2 = "";
                        }
                        yailItem.put(Component.LISTVIEW_KEY_IMAGE, str2);

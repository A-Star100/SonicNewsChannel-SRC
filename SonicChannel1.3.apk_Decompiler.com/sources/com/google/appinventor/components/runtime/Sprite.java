...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
    ...
    protected Sprite(ComponentContainer container, Handler handler) {
        ...
        if (container instanceof Canvas) {
            Canvas canvas2 = (Canvas) container;
            this.canvas = canvas2;
            canvas2.addSprite(this);
    ...
    public final void moveIntoBounds(int canvasWidth, int canvasHeight) {
        ...
        if (Width() > canvasWidth) {
            if (this.xLeft != 0.0d) {
                this.xLeft = 0.0d;
                this.xCenter = xLeftToCenter(0.0d);
                moved = true;
            }
            ...
            double Width = (double) (canvasWidth - Width());
            this.xLeft = Width;
            this.xCenter = xLeftToCenter(Width);
            moved = true;
        }
        if (Height() > canvasHeight) {
            if (this.yTop != 0.0d) {
                this.yTop = 0.0d;
                this.yCenter = yTopToCenter(0.0d);
                moved = true;
            }
            ...
            double Height = (double) (canvasHeight - Height());
            this.yTop = Height;
            this.yCenter = yTopToCenter(Height);
            moved = true;
        }
        if (moved) {
            registerChange();
    ...
    public void updateCoordinates() {
        ...
        double d4 = d + (d2 * d3);
        this.xLeft = d4;
        this.xCenter = xLeftToCenter(d4);
        double d5 = this.yTop;
        double d6 = (double) this.speed;
        double d7 = this.headingSin;
        Double.isNaN(d6);
        double d8 = d5 + (d6 * d7);
        this.yTop = d8;
        this.yCenter = yTopToCenter(d8);
    }
